  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        ...
        const response = await fetch(`/content/2.3-memoria-langchain.json?v=${timestamp}`, {
          cache: 'no-cache',
          headers: {
  ...
  if (loading) {
    return (
      <PageTemplate 
        ...
        subtitle="Sistema avançado de memória em LangGraph que permite agentes lembrarem interações passadas, aprenderem com feedback e adaptarem-se às preferências do usuário"
      >
