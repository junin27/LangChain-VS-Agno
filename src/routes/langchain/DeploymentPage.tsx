...
const DeploymentPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const response = await fetch('/content/2.8-implantacao-langchain.json', {
          headers: {
  ...
  if (loading) {
    return (
      <PageTemplate 
        ...
        subtitle="Estratégias e ferramentas para deploy de aplicações LangChain em diferentes ambientes, desde protótipos até produção em larga escala"
      >
