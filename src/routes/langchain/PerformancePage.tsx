...
const PerformancePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const response = await fetch('/content/2.7-desempenho-langchain.json', {
          headers: {
  ...
  if (loading) {
    return (
      <PageTemplate 
        ...
        subtitle="Análise de performance, overhead e estratégias de otimização para aplicações LangChain em produção"
      >
